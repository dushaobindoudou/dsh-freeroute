...
dsh 内置的 `设置 -> 模型` 页提供加法子插槽 `settings.models.footer`，本插件
注册进该插槽：完整面板作为「提供方行 + 添加区」之后的可折叠区块渲染，全程
不触碰宿主 DOM（插槽的普通内容，随设置页正常挂载/卸载）：

- 标题行 **免费模型 · FreeRoute** 点击折叠/展开（默认展开；收起即卸载面板，
  5s 状态轮询随之停止）；
- 展开后是完整 freeroute 面板——上游卡片（启停 / 优先级 / 申请教程 /
  密钥 / 连通测试 / 健康统计）、一键集成向导、远程目录配置、自定义上游表单
  （可接本地 uni-api / new-api / LiteLLM 自建网关）。

设置导航里没有独立的 freeroute 平级项（仅当宿主没有声明该子插槽、2.5s
宽限期后仍缺席时，才退回独立设置页）。`/freeproxy` 命令输出文本状态。
...
注意：端点随 `dsh web` 进程存活（同机使用）；至少在「设置 → 模型」页底部的
免费模型区块里给一个上游配好 Key，auto 链才有可用节点。
